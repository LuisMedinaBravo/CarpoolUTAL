...
export default {
  components: { loading },
    ...
    methods: {
        async login() {
           ...
           this.isLoading=true
            ...
            if (/^[A-Za-z0-9._%+-]+@utalca.cl$/.test(this.mail)) {
                ...
                if (resp == null) {
                    ...
                    localStorage.setItem('carpooltoken', JSON.stringify(resp));
                    this.$router.push({ path: '/menuadmincarpooling' })
                    this.$store.state.validlogin=true
                    localStorage.setItem('validlogin', JSON.stringify(this.$store.state.validlogin));
        ...
        openNotification(color) {
            if(color=='success'){
                const noti = this.$vs.notification({
                  color:'#27F055',
                  title: 'Exito al iniciar sesion',
                  text: `Los datos tan wenos🥶🥶🥶🥶`
                })
            }else{
                const noti = this.$vs.notification({
                  color,
                  title: 'Error al iniciar sesion',
                  text: `Los datos tan malos👺👺👺👺 `
                })
            }
        }
    },
    computed: {
        ...
        console.log(JSON.parse(localStorage.getItem("validlogin"))+"sdadasdsa")
        ...
        console.log(localStorage.getItem('validlogin')+"xx")
    }
};
</script>
<style scoped >
